  ...
  <WidgetWrapper id={id ? id : ''} hasBackground={hasBackground} containerClass="">
    <div className="row-gap-8 grid grid-cols-2 md:grid-cols-4">
      {items.map(({ title, description }, index) => (
        <div
          key={`item-stat-${index}`}
          className="mb-12 text-center md:mb-0 md:border-r md:last:border-none dark:md:border-zinc-500"
        >
          ...
          <p className="text-sm font-medium uppercase tracking-widest text-zinc-800 dark:text-zinc-400 lg:text-base">
            {description}
          </p>
        </div>
      ))}
